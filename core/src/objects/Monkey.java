...
public class Monkey extends AbstractGameObject 
{
	...
	public void init ()
	{
		...
		pe.load(Gdx.files.internal("particles/new.pfx"),
				Gdx.files.internal("particles"));
	...
	public void update (float deltaTime)
	{
		...
		if (timeLeftPineApplePowerup > 0)
		{
			...
			position.x += 1.0 * velocity.x * deltaTime;//double the speed when hasPineApple (Assignment 6 C)// brought it back to 1.5 becuz it was too fast

			pe.setPosition(position.x + dimension.x / 2,position.y+.55f);
			pe.start();
